#!/usr/bin/env python3
"""
train_clean_mlm_rae_v7_compagg_v9_entity_repr_anchor.py -- comp+agg with entity CE + representation anchoring
============================================================================
...
Key controls:
  ...
  - v8 adds an entity CE anchor: masked entity tokens can receive extra MLM
    cross-entropy weight, counteracting Para-induced smoothing on entity chains.
  - v8 can also turn Syn/Para off near the end for recovery-only MLM phases.
"""
...
for a in [
    ...
    ("--rae_syn_end_progress",float,-1.0,"If >=0, disable Syn at/after this training-progress fraction"),
    ...
    ("--rae_syn_exclude_syntax_after_syntax_para",bool,False,
     "After syntax Para starts, do not apply Syn loss to syntax tokens"),
    ("--rae_syn_syntax_after_syntax_para_scale",float,-1.0,
     "If >=0, scale Syn loss on syntax tokens after syntax Para starts"),
    ...
    ("--rae_para_content_end_progress",float,-1.0,"If >=0, disable content Para at/after this training-progress fraction"),
    ...
    ("--rae_para_syntax_end_progress",float,-1.0,"If >=0, disable syntax Para at/after this training-progress fraction"),
    ...
    ("--rae_para_orthogonal_to_syn",bool,False,
     "Project Para hidden/target away from attention-guided Syn context direction"),
    ("--rae_para_orthogonal_eps",float,1e-6,"Numerical floor for orthogonal Para projection"),
    ("--rae_syn_subspace",str,"",
     "Optional hidden-dim slice for Syn loss, e.g. 0:192. Empty uses all dims"),
    ("--rae_para_subspace",str,"",
     "Optional hidden-dim slice for Para loss, e.g. 192:384. Empty uses all dims"),

    # -- Entity anchor --
    ("--entity_ce_weight",float,1.0,"Extra MLM CE weight for masked entity tokens"),
    ("--entity_ce_start_progress",float,0.0,"Enable entity CE anchor after this training-progress fraction"),
    ("--entity_repr_anchor_weight",float,0.0,"Cosine anchor from masked entity hidden states to gold entity embeddings"),
    ("--entity_repr_anchor_start_progress",float,0.0,"Enable entity representation anchor after this training-progress fraction"),
    ("--entity_repr_anchor_end_progress",float,-1.0,"If >=0, disable entity representation anchor at/after this progress"),
]:
    n, t, d = a[0], a[1], a[2]
...
def _before_progress_end(cur_progress, end_progress):
    return end_progress < 0 or cur_progress < end_progress
...
def _select_hidden_subspace(x, spec):
    if not spec:
        return x
    if ":" not in spec:
        raise ValueError(f"Subspace spec must be start:end, got {spec!r}")
    left, right = spec.split(":", 1)
    start = int(left) if left else 0
    end = int(right) if right else x.shape[-1]
    if start < 0:
        start = x.shape[-1] + start
    if end < 0:
        end = x.shape[-1] + end
    start = max(0, min(start, x.shape[-1]))
    end = max(0, min(end, x.shape[-1]))
    if end <= start:
        raise ValueError(f"Invalid subspace {spec!r} for hidden size {x.shape[-1]}")
    return x[..., start:end]


# ======================== v7 核心: 注意力引导组合轴 ========================
def compute_syn_targets_attention(attentions, input_ids, labels, target_input_ids, emb_weight,
                                  pad_id, cls_id, sep_id, mask_id, args, device,
                                  syn_allowed_mask=None, exclude_labels=None):
    """Return attention-weighted Syn context targets for selected MLM rows."""
    mask = labels != -100
    ...
    exclude = exclude | (exclude_labels != -100)
    self_mask = torch.eye(L, device=device).unsqueeze(0).bool()
    exclude_3d = exclude.unsqueeze(1) | self_mask
    ...
    topk_scores, topk_idx = attn_sharp.topk(K, dim=-1)

    mask_idx = mask.nonzero(as_tuple=False)
    batch_idx = mask_idx[:, 0]
    seq_idx = mask_idx[:, 1]
    ctx_indices = topk_idx[batch_idx, seq_idx]
    ctx_scores = topk_scores[batch_idx, seq_idx]
    ...
    ctx_token_ids = target_input_ids[batch_idx.unsqueeze(-1), ctx_indices]
    ctx_embs = emb_weight[ctx_token_ids]
    syn_target = (ctx_weights.unsqueeze(-1) * ctx_embs).sum(dim=1)
    syn_target = F.normalize(syn_target.detach().float(), dim=-1)
    return batch_idx, seq_idx, syn_target
...
def compute_syn_loss_attention(hidden, attentions, input_ids, labels, target_input_ids, emb_weight,
    ...
    syn_target = _select_hidden_subspace(syn_target.detach().float(), args.rae_syn_subspace)
    h_masked = _select_hidden_subspace(hidden[batch_idx, seq_idx].float(), args.rae_syn_subspace)  # [M, Hs]
    syn_target = F.normalize(syn_target, dim=-1)
...
def compute_para_loss_neighbors(hidden, labels, emb_weight, args, device, orth_target=None):
    """
    ...
    para_target = _select_hidden_subspace(para_target.detach().float(), args.rae_para_subspace)
    h_float = _select_hidden_subspace(h_masked.float(), args.rae_para_subspace)

    if orth_target is not None:
        # Two-axis separation: Syn owns the attention-context direction; Para only
        # optimizes the residual neighbor-class component orthogonal to that axis.
        orth_target = _select_hidden_subspace(orth_target.detach().float(), args.rae_para_subspace)
        syn_dir = F.normalize(orth_target, dim=-1)
        h_perp = h_float - (h_float * syn_dir).sum(dim=-1, keepdim=True) * syn_dir
        p_float = para_target.float()
        p_perp = p_float - (p_float * syn_dir).sum(dim=-1, keepdim=True) * syn_dir

        eps = max(float(args.rae_para_orthogonal_eps), 1e-12)
        keep = (h_perp.norm(dim=-1) > eps) & (p_perp.norm(dim=-1) > eps)
        if keep.sum().item() == 0:
            return torch.tensor(0.0, device=device), 0
        h_norm = F.normalize(h_perp[keep], dim=-1)
        para_norm = F.normalize(p_perp[keep], dim=-1)
        cos_sim = (h_norm * para_norm).sum(dim=-1)
        para_loss = (1.0 - cos_sim).mean()
        return para_loss, int(keep.sum().item())

    para_target = F.normalize(para_target, dim=-1)
    h_norm = F.normalize(h_float, dim=-1)
...
def compute_weighted_mlm_loss(logits, labels, cm, entity_ce_weight):
    valid = labels != -100
    if valid.sum().item() == 0:
    ...
    weights = torch.ones_like(per_tok, dtype=torch.float32)
    if entity_ce_weight != 1.0:
        weights = torch.where(cm["entity"] & valid, weights * entity_ce_weight, weights)
    weights = weights.masked_fill(~valid, 0.0)
    loss = (per_tok * weights).sum() / weights.sum().clamp_min(1e-6)
    return loss, int((cm["entity"] & valid).sum().item())


def compute_entity_repr_anchor_loss(hidden, labels, cm, emb_weight, device):
    mask = (labels != -100) & cm["entity"]
    M = mask.sum().item()
    if M == 0:
        return torch.tensor(0.0, device=device), 0
    gold_ids = labels[mask]
    h = F.normalize(hidden[mask].float(), dim=-1)
    target = F.normalize(emb_weight[gold_ids].detach().float(), dim=-1)
    loss = 1.0 - (h * target).sum(dim=-1)
    return loss.mean(), int(M)
...
def train(args, model, tokenizer, train_dl, eval_dl):
    ...
    if syn_enabled:
        print(f"  Syn: attention-guided, layer={args.rae_syn_layer}, topk={args.rae_syn_topk}, "
              ...
              f"exclude_syntax_after_syntax_para={args.rae_syn_exclude_syntax_after_syntax_para}, "
              f"syntax_after_syntax_para_scale={args.rae_syn_syntax_after_syntax_para_scale}, "
              f"end_progress={args.rae_syn_end_progress}")
    else:
        print("  Syn: disabled")
    if para_enabled:
        print(f"  Content Para: embedding neighbors, topk={args.rae_para_topk}, "
              ...
              f"end_progress={args.rae_para_content_end_progress}")
        print(f"  Syntax Para: w={args.rae_para_syntax_weight}, gate_progress={psprog:.6f} "
              ...
              f"end_progress={args.rae_para_syntax_end_progress}")
        print(f"    Entity para={args.rae_para_entity_weight} Reading para={args.rae_para_reading_weight}")
        print(f"    Orthogonal-to-Syn Para={args.rae_para_orthogonal_to_syn}, "
              f"eps={args.rae_para_orthogonal_eps}")
        print(f"    Aux subspaces: Syn={args.rae_syn_subspace or 'all'} "
              f"Para={args.rae_para_subspace or 'all'}")
        print(f"  RAE constraints only on masked positions")
    else:
        print("  Para: disabled")
    print(f"  Entity CE anchor: weight={args.entity_ce_weight}, start_progress={args.entity_ce_start_progress}")
    print(f"  Entity repr anchor: weight={args.entity_repr_anchor_weight}, "
          f"start_progress={args.entity_repr_anchor_start_progress}, "
          f"end_progress={args.entity_repr_anchor_end_progress}")
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dl):
                ...
                for minibatch in batches:
                    ...
                    syn_active = syn_enabled and cur_progress >= sprog and _before_progress_end(cur_progress, args.rae_syn_end_progress)
                    syntax_para_active = (
                        para_enabled
                        and args.rae_para_syntax_weight > 0
                        and cur_progress >= psprog
                        and _before_progress_end(cur_progress, args.rae_para_syntax_end_progress)
                    )
                    content_para_active = (
                        para_enabled
                        and cur_progress >= pprog
                        and _before_progress_end(cur_progress, args.rae_para_content_end_progress)
                    )
                    orth_para_active = args.rae_para_orthogonal_to_syn and (syntax_para_active or content_para_active)

                    # 决定是否输出 attention（Syn loss 或正交 Para 都需要 Syn context direction）
                    need_attn = syn_active or orth_para_active

                    with torch.autocast(dtype=torch.bfloat16, device_type="cuda:0"):
                        o = model(**model_inputs, output_hidden_states=True,
                                  output_attentions=need_attn)
                        if args.entity_ce_weight != 1.0 and cur_progress >= args.entity_ce_start_progress:
                            loss, em = compute_weighted_mlm_loss(o.logits, labels, cm, args.entity_ce_weight)
                        else:
                            loss = o.loss
                            em = 0
                        mlm_l = loss.detach()
                        syn_l = para_l = para_s = ent_repr_l = torch.tensor(0.0, device=dev)
                        sm = pm = ps = erhm = 0

                        entity_repr_active = (
                            args.entity_repr_anchor_weight > 0
                            and cur_progress >= args.entity_repr_anchor_start_progress
                            and _before_progress_end(cur_progress, args.entity_repr_anchor_end_progress)
                        )
                        if entity_repr_active:
                            ent_repr_l, erhm = compute_entity_repr_anchor_loss(
                                o.hidden_states[-1], labels, cm, _ew, dev)
                            loss = loss + args.entity_repr_anchor_weight * ent_repr_l

                        layer_attn = o.attentions[args.rae_syn_layer] if o.attentions is not None else None  # [B, H, L, L]

                        # ── 组合轴 Syn: 注意力引导 ──
                        if syn_active and layer_attn is not None:
                            ...
                            if args.rae_syn_exclude_syntax_after_syntax_para and cur_progress >= psprog:
                                syn_allowed = syn_allowed & ~cm["syntax"]
                            elif args.rae_syn_syntax_after_syntax_para_scale >= 0 and cur_progress >= psprog:
                                if syn_weights is None:
                                    syn_weights = torch.ones_like(labels, dtype=torch.float32, device=dev)
                                syn_weights = torch.where(
                                    cm["syntax"],
                                    syn_weights * args.rae_syn_syntax_after_syntax_para_scale,
                                    syn_weights,
                                )
                                syn_weights = syn_weights.masked_fill(~cm["valid"], 0.0)
                        ...
                        if para_enabled and cur_progress >= pprog and _before_progress_end(cur_progress, args.rae_para_content_end_progress):
                        ...
                        if para_enabled and cur_progress >= min(pprog, psprog):
                            ...
                            if cur_progress >= pprog and content_para_scale > 0 and content_para_mask.sum() > 0:
                                ...
                                orth_target = None
                                if args.rae_para_orthogonal_to_syn and layer_attn is not None:
                                    ob, oseq, orth_target = compute_syn_targets_attention(
                                        layer_attn, input_ids, l_content, type_ids, _ew,
                                        tokenizer.pad_token_id, tokenizer.cls_token_id,
                                        tokenizer.sep_token_id, tokenizer.mask_token_id,
                                        args, dev, syn_allowed_mask=content_para_mask, exclude_labels=labels)
                                    if orth_target is not None:
                                        l_content_orth = torch.full_like(labels, -100)
                                        l_content_orth[ob, oseq] = labels[ob, oseq]
                                        l_content = l_content_orth
                                    else:
                                        l_content = torch.full_like(labels, -100)
                                para_l, pm = compute_para_loss_neighbors(
                                    h_content, l_content, _ew, args, dev, orth_target=orth_target)
                                loss = loss + args.rae_para_weight * content_para_scale * para_l

                            # Syntax tokens: 轻量 para
                            if args.rae_para_syntax_weight > 0 and cur_progress >= psprog and _before_progress_end(cur_progress, args.rae_para_syntax_end_progress):
                                ...
                                if syn_para_mask.sum() > 0:
                                    ...
                                    if args.rae_para_orthogonal_to_syn and layer_attn is not None:
                                        ob, oseq, orth_target = compute_syn_targets_attention(
                                            layer_attn, input_ids, l_syn, type_ids, _ew,
                                            tokenizer.pad_token_id, tokenizer.cls_token_id,
                                            tokenizer.sep_token_id, tokenizer.mask_token_id,
                                            args, dev, syn_allowed_mask=syn_para_mask, exclude_labels=labels)
                                        if orth_target is not None:
                                            l_syn_orth = torch.full_like(labels, -100)
                                            l_syn_orth[ob, oseq] = labels[ob, oseq]
                                            l_syn = l_syn_orth
                                        else:
                                            l_syn = torch.full_like(labels, -100)
                                    para_s, ps = compute_para_loss_neighbors(
                                        h_syn, l_syn, _ew, args, dev, orth_target=orth_target)
                ...
                if is_step("logging", gs, args):
                    ...
                    parts = [f"Ep{ep:.2f} MLM{mlm_l.item():.3f}"]
                    if args.entity_ce_weight != 1.0 and cur_progress >= args.entity_ce_start_progress:
                        parts.append(f"EntCEw{args.entity_ce_weight:.1f}(m{em})")
                    if args.entity_repr_anchor_weight > 0 and cur_progress >= args.entity_repr_anchor_start_progress:
                        parts.append(f"EntH{ent_repr_l.item():.4f}(m{erhm})")
                    ...
                    if cur_progress >= min(pprog, psprog) and para_enabled:
                        ...
                        if args.rae_para_syntax_end_progress >= 0 and cur_progress >= args.rae_para_syntax_end_progress:
                            parts.append("SynPoff")
                        if args.rae_para_content_end_progress >= 0 and cur_progress >= args.rae_para_content_end_progress:
                            parts.append("ParaOff")
                    parts.append(f"LR{sch.get_last_lr()[0]:.1e}")
                    print(" | ".join(parts), flush=True)
                    if args.wandb:
                        wd = {"train/epoch": ep, "train/mlm_loss": mlm_l.item(), "train/base_mlm_loss": o.loss.item(), "train/lr": sch.get_last_lr()[0]}
                        if args.entity_ce_weight != 1.0 and cur_progress >= args.entity_ce_start_progress:
                            wd["train/entity_ce_weight"] = args.entity_ce_weight; wd["train/entity_ce_masked"] = em
                        if args.entity_repr_anchor_weight > 0 and cur_progress >= args.entity_repr_anchor_start_progress:
                            wd["train/entity_repr_anchor_loss"] = ent_repr_l.item(); wd["train/entity_repr_anchor_masked"] = erhm
                        if cur_progress >= sprog and syn_enabled:
